...
class RegisterUserAccountServiceTest {
    ...
    void shouldReturnAuthResponseSuccessfully(){
        ...
        when(jwtTokenGeneratorPort.generateToken(any(Account.class))).thenReturn(new AuthToken("token", exp));

        AuthResponse response = registerUserAccountService.execute(command);

        assertNotNull(response);
        assertNotNull(response.accountId());
        assertEquals(email, response.email());
        assertEquals(token, response.jwtString());
        assertEquals(exp, response.expiresIn());
        ...
        verify(jwtTokenGeneratorPort, times(1)).generateToken(any(Account.class));
    }
    ...
    void shouldNotExecuteIfAccountWithMatchingEmailAlreadyExists(){
        ...
        verify(jwtTokenGeneratorPort, never()).generateToken(any(Account.class));
    }
    ...
    @Test
    void shouldNotContinueExecutingIfTokenGenerationProcessThrowsException(){
        when(accountRepositoryPort.findByEmail(email)).thenReturn(Optional.empty());
        when(passwordHasherPort.hash(password)).thenReturn(password + " hashed");
        when(accountRepositoryPort.save(any(Account.class))).thenAnswer(AdditionalAnswers.returnsFirstArg());
        when(jwtTokenGeneratorPort.generateToken(any(Account.class))).thenThrow(new RuntimeException("Error in the token generation process"));
