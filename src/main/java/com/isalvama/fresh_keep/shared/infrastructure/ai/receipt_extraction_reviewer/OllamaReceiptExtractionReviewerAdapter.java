...
public class OllamaReceiptExtractionReviewerAdapter implements AiReceiptExtractionReviewerPort {
    ...
    private static final String TEMPLATE_PROMPT_TEXT = """
            ...
            2. Expiration date: is "expirationDate" a plausible shelf life for this kind of product, counting from {shoppingDate}? Flag it if it is clearly wrong for the product type - e.g. a fresh product expiring years from now, a frozen or shelf-stable product expiring in a day or two, or a date already in the past.
    ...
    public List<ProductReviewFlag> review(ReviewNewShoppingReceiptDto dto) {
        ...
        try {
            response = chatModel.call(new Prompt(buildPrompt(dto, converter), chatOptions));
    ...
    private String buildPrompt(ReviewNewShoppingReceiptDto dto, BeanOutputConverter<ReceiptExtractionToReview> converter){
        String storageSpotsList = dto.storageSpots().stream()
                .map(s -> "- id: %s, name: %s, type: %s".formatted(s.id(), s.name(), s.type()))
                .collect(Collectors.joining("\n"));

        PromptTemplate promptTemplate = new PromptTemplate(TEMPLATE_PROMPT_TEXT);
